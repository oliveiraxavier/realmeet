...
public class AllocationValidator {
    ...
    private void validateDateOrdering(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
        if (!endAt.isAfter(startAt)) {
            validationErrors.add(ALLOCATION_START_AT, ALLOCATION_START_AT + INCORRECT_DATES);
        }
    }

    private void validateDateInTheFuture(OffsetDateTime startAt, ValidationErrors validationErrors) {
    ...
    private void validateDuration(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
        if (Duration.between(startAt, endAt).getSeconds() > ALLOCATION_MAX_DURATION) {
            validationErrors.add(ALLOCATION_END_AT, ALLOCATION_END_AT + ALLOCATION_EXCEEDS_DURATION);
        }
    }
    ...
    private void validateIfTimeAvailable(
        ...
        //TODO : validar datas de acordo com as existentes na tabela
    }
}
